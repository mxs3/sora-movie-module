...
async function extractDetails(id) {
    try {
        const response = await fetch(`https://animeflv.ahmedrangel.com/api/v1/anime/${id}`);
        const data = await response.json();
...
async function extractEpisodes(id) {
    try {
        const response = await fetch(`https://animeflv.ahmedrangel.com/api/v1/anime/${id}/episodes`);
        const data = await response.json();
...
async function extractStreamUrl(url) {
    try {
        ...
        const response = await fetch(`https://animeflv.ahmedrangel.com/api/v1/anime/${animeID}/episode/${episodeID}/streams`);
        const data = await response.json();
